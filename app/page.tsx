...
export default function Home() {
  ...
  const [improveCooldown, setImproveCooldown] = useState(false);
  ...
  const handleImprove = async () => {
    ...
    try {
      ...
      setError("Failed to improve transcript");
    } finally {
      setIsImproving(false);
      setImproveCooldown(true);
      setTimeout(() => setImproveCooldown(false), 15000);
    }
  };
            ...
            <div>
              ...
              <div className="flex items-center justify-between mb-2">
                {isImproved ? (
                  <button
                    onClick={handleUndo}
                    className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors border border-slate-300 text-slate-700 hover:bg-slate-50 active:bg-slate-100"
                  >
                    <svg className="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path
                        strokeLinecap="round"
                  ...
                  <button
                    ...
                    disabled={isImproving || improveCooldown}
                    className="flex items-center gap-2 px-4 py-2 rounded-lg text-sm font-medium transition-colors border border-purple-300 text-purple-700 hover:bg-purple-50 active:bg-purple-100 disabled:opacity-50 disabled:cursor-not-allowed"
                  >
                    {isImproving ? (
